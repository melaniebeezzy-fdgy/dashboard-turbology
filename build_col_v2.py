...
# ---- dashboard más reciente (20-jul..10-ago, RTWT + coverageCurrent) ----
UP4 = 'foodology_rt_dashboard_v4.html'
up4 = {}
if os.path.exists(UP4):
    _h4 = open(UP4, encoding='utf-8', errors='replace').read()
    for _m in re.finditer(r'"storeId"', _h4):
        _i = _m.start(); _s = _h4.rfind('{', 0, _i); _d = 0; _e = None
        for _j in range(_s, len(_h4)):
            if _h4[_j] == '{': _d += 1
            elif _h4[_j] == '}':
                _d -= 1
                if _d == 0: _e = _j + 1; break
        try: _o = json.loads(_h4[_s:_e])
        except: continue
        if 'w4' in _o and 'storeId' in _o:
...
# ---- dashboard más reciente (03-ago..24-ago, RTWT + coverageCurrent) ----
UP6 = 'foodology_rt_dashboard_v6.html'
up6 = {}
if os.path.exists(UP6):
    _h6 = open(UP6, encoding='utf-8', errors='replace').read()
    for _m in re.finditer(r'"storeId"', _h6):
        _i = _m.start(); _s = _h6.rfind('{', 0, _i); _d = 0; _e = None
        for _j in range(_s, len(_h6)):
            if _h6[_j] == '{': _d += 1
            elif _h6[_j] == '}':
                _d -= 1
                if _d == 0: _e = _j + 1; break
        try: _o = json.loads(_h6[_s:_e])
        except: continue
        if 'w4' in _o and 'storeId' in _o:
